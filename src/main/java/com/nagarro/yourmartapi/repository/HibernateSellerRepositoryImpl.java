...
public class HibernateSellerRepositoryImpl implements SellerRepository {
	...
	@Override
	public List<Seller> getAllSeller(int offset, int limit, String sortBy, String filter, String searchKey,
			String searchQuery) {
		boolean isWhereRequired = true;
		String queryString = "SELECT s FROM Seller s ";

		if (searchKey != null) {
			String sq = searchKey.equals("COMPANY_NAME") ? "s.companyName"
					: (searchKey.equals("OWNER_NAME") ? "s.ownerName" : "s.phone");
